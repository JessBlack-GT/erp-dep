...
describe('M06 registration in MainNavigator (native navigation mocked)', () => {
  it('registers all product screens for an authenticated session', () => {
    useAuth.mockReturnValue({ isAuthenticated: true, loading: false });
    const screen = render(<MainNavigator />);
    for (const name of ['Inventory', 'InventoryMovementForm', 'WarehouseForm'])
  ...
  it('does not expose product screens while unauthenticated or loading', () => {
    ...
    const screen = render(<MainNavigator />);
    expect(screen.queryByText('Inventory:function')).toBeNull();
